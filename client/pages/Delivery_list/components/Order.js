...
export default function Order(props) {
  ...
  useEffect(() => {
    if(order_delivery_price && order_total_price && data){
      // router.replace({
      //   pathname: router.asPath,
      //   query: {  : order_total_price},
      // },
      // undefined,
      // { shallow: true });

      router.push(
        `?or-pr_${data.order_id}=${order_total_price}&d-pr_${data.order_id}=${order_delivery_price}`,
      );
    }
  }, [order_delivery_price, order_total_price, data]);
