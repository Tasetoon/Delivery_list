...
export default function Order(props) {
  ...
  <div className={order_style}>
    ...
    <ul className='mt-3'>
      <li className="w-full ">Доп. контакты: {data.additional_contacts}</li>

      <li className="w-full ">
            <a href={adress_yandex_map}>Адрес: {data.adress}</a>
      </li>
      <li className="w-full ">{paid}{extra}</li>
      <li className="w-full ">клиент: {data.customer}</li>
      <li className="w-full ">метро: {data.metro}</li>
      <li className="w-full ">
        <a href={phone}>{data.phone_number}</a>
      </li>
      <li className="w-full ">время доставки: {data.tovar_arrival_time}</li>
      <li className="w-full ">
        {positions.length > 0 ? (  
          <div>
            {positions.map(d => (
