...
export default function Order(props) {
  ...
  const [order_style, setOrderStyle] = useState('m-2 flex flex-col font-medium rounded-lg order section');
  ...
  <div className={order_style}>
    ...
    <ul className='mt-3'>
      <li key={'additional_contacts'} className="w-full list-element separetor text-xl">Доп. контакты: {data.additional_contacts}</li>

      <li key={'adress'} className="w-full list-element separetor">
        <div className='flex text-xl'>
          <LocationLogo/>
          <a className=' link' href={adress_yandex_map}>Адрес: {data.adress}</a>
        </div>
      </li>
      <li key={'paid'} className="w-full list-element separetor">
        {paid === 'Оплачено' ? (
          <div className='flex text-xl'>
            <CheckMarkLogo/>
            <p>{paid}{extra}</p>
      ...
      <li key={'customer'} className="w-full list-element separetor">
        <div className='flex text-xl'>
          <ClientLogo/>
      ...
      <li key={'metro'} className="w-full list-element separetor">
        <div className='flex text-xl'>
          <MetroLogo/>
          <p className=''>Метро: {data.metro}</p>
        </div>
      </li>
      <li key={'phone_number'} className="w-full list-element separetor">
        <div className='flex text-xl'>
          <PhoneCallLogo/>
          <a href={phone}>{data.phone_number}</a>
        </div>
      </li>
      <li key={'tovar_arrival_time'} className="w-full list-element separetor">
        <div className='flex text-xl'>
          <ClockLogo/>
